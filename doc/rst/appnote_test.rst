...
The ``appnote_test`` software zip-file should be downloaded and unzipped to a chosen directory.
...
`app_appnote_test/bin` directory, one for each of the build configurations.

===================
Running the example
===================

From a XTC command prompt run the following command from the `appnote_test/app_appnote_test` directory::

    xrun ./bin/app_appnote_test.xe

Alternatively the binary can be programmed into the non-volatile flash memory with the command::

    xflash ./bin/app_appnote_test.xe
